            ...
            open_manipulator_msgs::setxyz::Response &res)
{
  ...
  res.srv_output_x=req.srv_input_x;
  res.srv_output_y=req.srv_input_y;
  res.srv_output_z=req.srv_input_z;

  input_x=res.srv_output_x;
  input_y=res.srv_output_y;
  input_z=res.srv_output_z;
  printf("bye\n");
  printf("x=%.3lf, y=%.3lf, z=%.3lf\n",input_x,input_y,input_z);
  kinematics_pose.push_back(input_x);
  kinematics_pose.push_back(input_y);
  kinematics_pose.push_back(input_z);
/////////////////
  open_manipulator_msgs::SetKinematicsPose srv2;

  srv2.request.end_effector_name = "gripper";

  srv2.request.kinematics_pose.pose.position.x = kinematics_pose.at(0);
  srv2.request.kinematics_pose.pose.position.y = kinematics_pose.at(1);
  srv2.request.kinematics_pose.pose.position.z = kinematics_pose.at(2);

  srv2.request.kinematics_pose.pose.orientation.w = kinematics_pose_.pose.orientation.w;
  srv2.request.kinematics_pose.pose.orientation.x = kinematics_pose_.pose.orientation.x;
  srv2.request.kinematics_pose.pose.orientation.y = kinematics_pose_.pose.orientation.y;
  srv2.request.kinematics_pose.pose.orientation.z = kinematics_pose_.pose.orientation.z;

  srv2.request.path_time = path_time;
  //srv.request.path_time = PATH_TIME;
    printf("teleop21\n");
  if(goal_task_space_path_position_only_client_.call(srv2))
  { printf("teleop2\n");
    return srv2.response.is_planned;
  }
  printf("teleop23\n");
  return false;
...
//////////////////
/*
  if(!setTaskSpacePath(kinematics_pose, path_time)) {//return;
   printf("x=%.3lf, y=%.3lf, z=%.3lf\n",input_x,input_y,input_z);
   }
  return true;
  */
...
else if (open_ch_subnum ==3)
{
  printf("input : a \tincrease(++) y axis in task space\n");
  goalPose.at(1) = 5*DELTA;
  setTaskSpacePathFromPresentPositionOnly(goalPose, PATH_TIME);
}
else if (open_ch_subnum ==4)
{
  printf("input : d \tdecrease(--) y axis in task space\n");
  goalPose.at(1) = -5*DELTA;
  setTaskSpacePathFromPresentPositionOnly(goalPose, PATH_TIME);
}
else if (open_ch_subnum ==5)
{
  printf("input : z \tincrease(++) z axis in task space\n");
  goalPose.at(2) = 5*DELTA;
  //goalPose.at(2) = -DELTA;
  setTaskSpacePathFromPresentPositionOnly(goalPose, PATH_TIME);
}
else if (open_ch_subnum ==6)
{
...
void OpenManipulatorTeleop::setGoal(char ch)
{
  ...
  { //kudos input
    ...
    while(1){
    printf("scan x y z\n");
    scanf("%lf %lf %lf",&input_x,&input_y,&input_z);
    printf("x=%.3lf, y=%.3lf, z=%.3lf\n",input_x,input_y,input_z);
    break;
    }*/
    printf("hi\n");
    float input_x,input_y,input_z;
    open_manipulator_msgs::setxyz srv;
    //ros::ServiceServer setxyzservice=node_handle_.advertiseService("input_xyz",setxyz_);
    //ros::spinOnce();

    //input_x=srv.response.srv_output_x;
    //input_y=srv.response.srv_output_y;
    //input_z=srv.response.srv_output_z;
    //printf("x=%.3lf, y=%.3lf, z=%.3lf\n",input_x,input_y,input_z);
    //if(input_x==0.7)break;
    printf("bye\n");
    printf("x=%.3lf, y=%.3lf, z=%.3lf\n",input_x,input_y,input_z);
    kinematics_pose.push_back(input_x);
    kinematics_pose.push_back(input_y);
    kinematics_pose.push_back(input_z);
    //goalPose.push_back(input_x);
    //goalPose.push_back(input_y);
    //goalPose.push_back(input_z);
    //OpenManipulatorTeleop().setTaskSpacePath(goalPose, PATH_TIME);
    //if(!OpenManipulatorTeleop().setTaskSpacePath(kinematics_pose, path_time));
    //if(!OpenManipulatorTeleop().setTaskSpacePath(goalPose, path_time)) {return;
    if(!setTaskSpacePath(kinematics_pose, path_time)) {return;
      //printf("x=%.3lf, y=%.3lf, z=%.3lf\n",input_x,input_y,input_z);
      //여기는 원래코드도 실행안됨 printf("kudos_main_window23\n"); writeLog("[ERR!!] Failed to send service");
      //여기가 뜬다는건 오류가났다는거
     }
 ...
 /* else if (ch == 'n' || ch == 'N')
  {
    printf("input : n \tmove CCW(--) roll joint joint5 angle\n");

    DynamixelWorkbench dxl_wb;
    uint8_t id=16;
    int32_t delta_value=100;
    const char *log;
    int dxl_id = 16;
    int32_t get_data = 0;
  ...
  { printf("Succeed to get present position(value : %d)\n", get_data);  }
    ...
    int32_t new_data=get_data+delta;
    dxl_wb.goalPosition(dxl_id,new_data);
  }

  else if (ch == 'm' || ch == 'M')
  {
    printf("input : n \tmove CW(++) roll joint joint5 angle\n");
...
int main(int argc, char **argv)
  // Init ROS node
{
  ros::init(argc, argv, "open_manipulator_teleop_keyboard_woo");
  OpenManipulatorTeleop openManipulatorTeleop;

  ros::NodeHandle node_handle_;
  ros::ServiceServer setxyzservice=node_handle_.advertiseService("input_xyz",setxyz_);

  //ros::Subscriber open_topic_sub_woo=node_handle_.subscribe("opentopic",1000,openManipulatorTeleop->opensubCallback);//kudossub
