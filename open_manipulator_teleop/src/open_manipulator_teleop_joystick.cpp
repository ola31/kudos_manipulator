...
void OpenManipulatorTeleop::initSubscriber()
{
  ...
  manipulator_teleop_onoff_sub = node_handle_.subscribe("/teleop_onoff", 10, &OpenManipulatorTeleop::teleoponoffCallback , this); //on off
}
...
void OpenManipulatorTeleop::joyCallback(const sensor_msgs::Joy::ConstPtr &msg)
{
  ...
  else if (msg->axes.at(4) >=  0.9) setGoal("z+");
  else if (msg->axes.at(4) <= -0.9) setGoal("z-");
  else if (msg->buttons.at(5) == 1) setGoal("home");
  else if (msg->buttons.at(4) == 1) setGoal("init");

  else if (msg->axes.at(2) <=  -0.9) setGoal("gripper open");
  else if (msg->axes.at(5) <=  -0.9) setGoal("gripper close");
  else if (msg->axes.at(7) ==  1.0) setGoal("joint4+");
  else if (msg->axes.at(7) == -1.0) setGoal("joint4-");
  }
}
//on off
...
void OpenManipulatorTeleop::setGoal(const char* str)
{
  ...
  else if (str == "joint4+")
  {
    ...
    joint_name.push_back("joint4"); goalJoint.at(3) = 1*JOINT_DELTA;
