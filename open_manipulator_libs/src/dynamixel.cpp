...
void JointDynamixel::setMode(std::vector<uint8_t> actuator_id, const void *arg)
{
  ...
  if (get_arg_[0] == "position_mode" || get_arg_[0] == "current_based_position_mode")
  {
    ...
    if(get_arg_[0]=="position_mode"){
      result = JointDynamixel::setOperatingMode(actuator_id, "extended_position_mode");
    }
    else if(get_arg_[0]=="current_based_position_mode"){
      result = JointDynamixel::setOperatingMode(actuator_id, get_arg_[0]);
    }
...
void JointDynamixel::enable()
{
  ...
  result = dynamixel_workbench_->setOperatingMode(11,4,NULL);
  result = dynamixel_workbench_->setOperatingMode(12,4,NULL);
  result = dynamixel_workbench_->setOperatingMode(13,4,NULL);

  result = dynamixel_workbench_->setOperatingMode(14,4,NULL);
...
{
  JointDynamixelProfileControl::disable();

  const char* log = NULL;
  bool result = false;
  result = dynamixel_workbench_->setOperatingMode(11,4,&log);
  if (result == false)
  {
    log::error(log);
  }
  result = dynamixel_workbench_->setOperatingMode(12,4,&log);
  if (result == false)
  {
    log::error(log);
  }
  result = dynamixel_workbench_->setOperatingMode(13,4,&log);
  if (result == false)
  {
    log::error(log);
  }
  result = dynamixel_workbench_->setOperatingMode(14,4,&log);
  if (result == false)
  {
    log::error(log);
  }
